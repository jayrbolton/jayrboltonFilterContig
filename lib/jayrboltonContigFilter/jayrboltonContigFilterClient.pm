package jayrboltonContigFilter::jayrboltonContigFilterClient;
...
jayrboltonContigFilter::jayrboltonContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => jayrboltonContigFilter::jayrboltonContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
=head2 filter_contigs
  ...
  $return = $obj->filter_contigs($workspace_name, $params)
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jayrboltonContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "jayrboltonContigFilter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "jayrboltonContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for jayrboltonContigFilter::jayrboltonContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "jayrboltonContigFilter::jayrboltonContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package jayrboltonContigFilter::jayrboltonContigFilterClient::RpcClient;
